...
transport_model <- ompr::MIPModel() %>%
  add_variable(flow[supplierindex,customerindex],supplierindex=1:supplier_count,
               customerindex=1:customer_count,type='continuous',lb=0) %>%
  ...
  add_constraint(sum_expr(flow[supplierindex,customerindex],supplierindex=1:supplier_count) ==
                   customer_df$Demand[customerindex], customerindex=1:customer_count) %>%
